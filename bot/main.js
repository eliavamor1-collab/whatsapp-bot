...
import wedjCommand from "./commands/wedj.js";
...
import kinestopCommand from "./commands/kinestop.js";
import movitCommand from "./commands/movit.js";
...
registerCommand(kinestopCommand);
registerCommand(movitCommand);
  ...
  try {
    ...
    const socketConfig = {
      ...
      getMessage: async (key) => {
        // מחזירים undefined כשאין לנו את ההודעה — Baileys יבטל את ה-retry
        // ולא ישלח הודעה ריקה כ-retry response
        return undefined;
      },
      ...(version && { version })
    };
    ...
    // ========================================
    // Guard — מונע שליחת הודעות ריקות
    // עוטף את sendMessage ובודק את התוכן לפני כל שליחה
    // ========================================
    const _originalSendMessage = sock.sendMessage.bind(sock);
    sock.sendMessage = async (jid, content, opts) => {
      // בודק שיש תוכן ממשי לפני שליחה
      const text = content?.text ?? content?.caption ?? content?.image?.caption ?? null;
      const hasMedia = content?.image || content?.video || content?.audio ||
                       content?.document || content?.sticker || content?.forward;

      if (!hasMedia && typeof text === "string" && text.trim() === "") {
        console.warn(`[SendGuard] ⛔ נחסמה שליחת הודעה ריקה ל-${jid}`);
        return null;
      }

      return _originalSendMessage(jid, content, opts);
    };
  ...
  if (req.url === "/update-notification" && req.method === "POST") {
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        // אם לא מחובר — מנסה עד 10 פעמים עם המתנה של 10 שניות
        if (!sock || !currentStatus.includes("מחובר")) {
          console.log(`[UpdateNotify] WhatsApp לא מחובר — ממתין לחיבור ושולח ${appName}...`);
          let connected = false;
          for (let i = 0; i < 10; i++) {
            await new Promise(r => setTimeout(r, 10000));
            if (sock && currentStatus.includes("מחובר")) {
            ...
            console.log(`[UpdateNotify] עדיין לא מחובר (ניסיון ${i + 1}/10)...`);
          }
          if (!connected) {
            console.log(`[UpdateNotify] ❌ לא הצליח להתחבר — מדלג על עדכון ${appName}`);
            return;
          }
          console.log(`[UpdateNotify] ✅ מחובר — שולח עדכון ${appName}`);
        }
